...
(function () {
  ...
  function render() {
    ...
    window.SD_ROUTE = r.file;
    document.title = page.title;
    document.getElementById('sd-page-style').textContent = page.styles || '';
    ...
    try {
      ...
      console.error('[system-design] runtime failed on ' + r.file, e);
    }
    if (window.SD) window.SD.go = go;

    for (var i = 0; i < page.scripts.length; i++) {
      try {
        new Function(page.scripts[i])();
      } catch (e) {
        console.error('[system-design] page script ' + (i + 1) + ' failed on ' + r.file, e);
      }
    }
