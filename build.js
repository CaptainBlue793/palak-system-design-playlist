...
/* =========================================================
   ...
   <script> is captured into window.SD_PAGES. A tiny hash router
...
const OUT_FILE = path.join(OUT_DIR, 'system-design-course.html');
...
const THEME_BOOTSTRAP =
  `try{var t=JSON.parse(localStorage.getItem('sd-theme'));if(t)document.documentElement.setAttribute('data-theme',t)}catch(e){}`;
...
appJs = patch(appJs, 'assets/app.js',
  ...
  `function go(r) { if (r) { close(); SD.go(r.f, r.a); } }`);
appJs = patch(appJs, 'assets/app.js',
  `const here = location.pathname.split('/').pop();`,
  `const here = window.SD_ROUTE || location.pathname.split('/').pop();`);

pages['index.html'].scripts = pages['index.html'].scripts.map((s) => patch(
  s, 'index.html',
  `n.addEventListener('click', () => location.href = ch.file);`,
  `n.addEventListener('click', () => SD.go(ch.file));`,
  /* optional */ true));
if (!pages['index.html'].scripts.some((s) => s.includes('SD.go(ch.file)'))) {
...
<script>window.SD_PAGES = ${json(pages)};</script>
<script>${studyData}</script>
<script>window.__SD_BOOT = function () {\n${appJs}\n};</script>
